...
#define BUILD_TOPN(funcname)                                          \
  planBuilder.values(values)                                          \
      .funcname(partitionKeys, sortingKeys, limit, generateRowNumber) \
      .planNode()

#define BUILD_TOPN_PLANNODEID(funcname)                               \
  planBuilder.values(values)                                          \
      .funcname(partitionKeys, sortingKeys, limit, generateRowNumber) \
      .capturePlanNodeId(planNodeId)                                  \
      .planNode()

#define BUILD_TOPN_PARTIAL_FINAL(funcname)                \
  planBuilder.values(values)                              \
      .funcname(partitionKeys, sortingKeys, limit, false) \
      .capturePlanNodeId(planNodeId)                      \
      .funcname(partitionKeys, sortingKeys, limit, true)  \
      .planNode()
  ...
  auto data = split(
      makeRowVector(
          {"d", "s", "p"},
          {
              ...
              // Partitioning key. Each partition has 1000 rows.
              makeFlatVector<int64_t>(
